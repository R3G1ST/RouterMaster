...
Полный сброс роутера → роутер перезагружается (до 10 минут) → пароль устанавливается автоматически → автоматически выполняется полная настройка (Wi-Fi, Podkop, тема, язык). Роутер вернётся полностью чистым и настроенным.
